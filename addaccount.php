...
if($_SERVER['REQUEST_METHOD']=="POST"){ 
    ...
    if(!empty($_POST['Username'])&&!empty($_POST['Pass'])&&!empty($_POST['EMail'])&&!empty($_POST['Usertype'])&&!empty($_POST['Pnumber'])&&!empty($_POST['firstname'])&&!empty($_POST['lastname'])&&!empty($_POST['adress']))
    {
        ...
        if(!empty($USERname)&&!empty($FirstName)&&!empty($LastName)&&!empty($PASSword)&&!empty($Email)&&!empty($adress)&&!empty($UserType)&&!empty($TELnumber)&&empty($Email_err)&&empty($Username_err))
            {
                $data[]=array("NO."=>$NextId,"firstname"=>$FirstName,"lastname"=>$LastName,"UserName"=>$USERname,"EMail"=>$Email,"PassWord"=>$PASSword,"UserTYpe"=>$UserType,"tel"=>$TELnumber,"adress"=>$adress,"city"=>$city,"unique-id"=>$random_userid);
...
<body>
    <nav class="navbar">
        <li type="none" style="display: inline-block;"><a href="index.php"><img class="imgsize2" src="ezgif.com-resize (1).gif" alt="Sيdiliti" title="Home"></li>
        <!--home page link --><li type="none" class="liorder"><a href="index.php"><button class="navbutton" title="home">Home</button></a></li>
        ...
        <div class="container">
            <form action="signup2.php" method="post">
                <div class="step step-1 active">
                    ...
                    <button type="button" class="nbtn" title="Next">Next</button>
                </div>
                <div class="step step-2">
                   ...
                   <button type="button" class="pbtn" title="Previous">Prev</button>
                   <button type="button" class="nbtn" title="Next">Next</button>
                </div>
                ...
                <div class="step step-4 ">
                    ...
                    <button type="button" class="pbtn" title="Previous">Prev</button>
                    <!-- <input class="sbtn" value="submit"  type="submit" title="Submit"> -->
                    <!-- <a href="LOGIN.php" class="signinbtn" title="Log In">Sign in</a> -->
                </div>
            </form>
        </div>
    </section>
